...
namespace OnlineTravel.Application.Features.Tours.CreateTour.Validators
{
    public class CreateTourCommandValidator : AbstractValidator<CreateTourCommand>
    {
        public CreateTourCommandValidator()
        {
            ...
            RuleFor(p => p.MainImageUrl)
                .Must(uri => Uri.TryCreate(uri, UriKind.Absolute, out _)).When(x => !string.IsNullOrEmpty(x.MainImageUrl))
